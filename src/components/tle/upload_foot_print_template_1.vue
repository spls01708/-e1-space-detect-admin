<template>
    ...
    <main class="site-wrapper">
                                    ...
                                    <b-row class="w-100 text-center mt-0 " >
                                      ...
                                      <b-col>
                                        ...
                                        <div  v-for="(field, index) in fields" :key="index">
                                          ...
                                          <b-row v-if="field.footprint_label != ''" style="margin-top:-1em;">
                                            <b-col sm=4>
                                              <b-form-checkbox size="sm" style="float:right;margin-top:1.8em;margin-right:-1em;" v-model="field.selected">{{field.footprint_label}}</b-form-checkbox>    
                                            </b-col>
                                            <b-col>
                                              <div class="box" style=""> 
                                                    <i class="fa fa-paperclip" aria-hidden="true"></i> Footprint file : 
                                                    <span style="font-weight:bold;cursor:pointer;text-decoration: underline;text-decoration-style: double;"><a :href="field.link" target="_blank" rel="noopener noreferrer">
                                                      click</a></span> 
                                              </div>
                                            </b-col>
                                          </b-row>

                                          <b-row v-else style="margin-top:0.5em;margin-bottom:-0.5em;">
                                            <b-col sm=4>
                                              <b-form-checkbox size="sm" style="float:right;margin-top:3px;" v-model="field.selected">{{field.label}}</b-form-checkbox>    
                                            </b-col>
                                            <b-col>
                                      ...
                                      <b-row style="zoom:0.9;"  >
                                        ...
                                        <b-col>
                                          <button class="  btn btn-outline-primary rounded-0 w-100 border-2 " @click="addRow"  > 
                                                <i class="fa fa-plus-circle" aria-hidden="true"></i><span style="margin-left:1em;"> Add Footprint </span>
                                          </button> 
                                        </b-col>

                                        <b-col>
                                          <button class="  btn btn-outline-danger rounded-0 w-100 border-2 "  @click="delRow" > 
                                                <i class="fa fa-minus-circle" aria-hidden="true"></i><span style="margin-left:1em;"> Del Footprint </span>
                                          </button> 
                                        </b-col>
      ...
      <b-modal v-model="progress_open" size="lg"  centered no-close-on-backdrop hide-header hide-footer  hide-header-close  title="info">
        ...
        <b-modal v-model="change_footprint_name" size="l"  centered no-close-on-backdrop hide-header hide-header-close  title="Login" >
            ...
            <b-row class="mt-2" align-v="center" >
                ...
                <b-col  sm="4">
                  ...
                  <span class="text-dark" style="float:right;"> Footprint Name  : </span>
                 
                </b-col>
                <b-col  sm="8">
                <b-form-input
                    id="filter-input"
                    v-model="footprint_name"
                    type="text"
                    autocomplete="off"
                    
                    style=" cursor:pointer;"
                    class="rounded-0 border-2"
                    :state="footprint_name.length>0"
                ></b-form-input>
            ...
            <template v-slot:modal-footer="{ }">  
                ...
                <button class="  btn btn-outline-success rounded-0 border-2 w-75" @click="fn_change_footprint_name()" :disabled="footprint_name.length==0">
                  <i class="fa fa-check" aria-hidden="true"></i> OK
                </button>

                <button class="  btn btn-outline-danger rounded-0 border-2 "  @click="fn_change_footprint_close()" >
...
export default {
  ...
  data() {
    ...
    methods: {
        fn_change_footprint_name () {

          this.fields.push({
            label: this.footprint_name,
            file: null,
            selected : false,
            id : this.getId(),
          ...
          this.change_footprint_name = false

        },
        fn_change_footprint_close () {

          this.change_footprint_name = false
        },
        addRow: function() {
           
          this.change_footprint_name = true
          this.footprint_name = ''
        },

        async fn_remove_footprint_file( norad_catalog_number, footprint_label ){
                      ...
                      return new Promise((resolve, reject) => { 
                                ...
                                let end_point = myConfig.server_domain+"/v1/tle/footprint/remove/file"
        ...
        async delRow () {
          ...
          this.fields.forEach( async (v, i, a) => { 

            if(v.selected == true){
              try{
                  await this.fn_remove_footprint_file(this.sat_info.norad_catalog_number, v.footprint_label)
              }catch(e){}
         ...
         onChange( field ) {
            ...
            let xfile = field.file 
            // console.log( xfile )

            let formData = new FormData();
            formData.append('footprint_file', xfile);
            formData.append('footprint_file_name', xfile.name);
            formData.append('footprint_label', field.label);

            formData.append('orbit_position', this.footprint_position_degree);

            formData.append('norad_catalog_number', this.sat_info.norad_catalog_number);
            formData.append('name', this.sat_info.name);

            formData.append('sat_info', JSON.stringify( this.sat_info ));
            // --------------------------------------------------------------------------------- 
            ...
            let end_point = myConfig.server_domain+"/v1/tle/footprint/add/file"
            ...
            .then( async (res) => {
              ...
              if( res.data.result == 'Process-Complete' ){

                this.fields.forEach( async (v, i, a) => { 

                  if(v.id == field.id){
 
                    v.footprint_label = field.label
                    v.footprint_file_name = xfile.name
                    v.footprint_file_link = myConfig.server_domain+"/v1/footprint/kml/download/"+file_name 

                    this.fields.splice( i, 1, v )
    ...
    mounted() {
        ...
        if( this.tle_data.item.orbit_footprint ){

          // console.log( this.tle_data.item.orbit_footprint )
          this.fields = []

          for(let i=0;i<this.tle_data.item.orbit_footprint.length;i++){

              let _obj = {
              
                  label : this.tle_data.item.orbit_footprint[i].name,
                  file : null,
                  selected : false,
                  id : this.getId(),

                  footprint_label : this.tle_data.item.orbit_footprint[i].footprint_label,
                  footprint_file_name : this.tle_data.item.orbit_footprint[i].footprint_file_name,
                  footprint_file_link : this.tle_data.item.orbit_footprint[i].footprint_file_link,
              
              }

              this.fields.push(_obj)
