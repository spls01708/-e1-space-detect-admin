<template>
    ...
    <main class="site-wrapper" style="margin-top:2em;">
      ...
      <div class="media-body text-left ml-0 align-self-center" style="zoom:0.9;margin-top:1em;height:83vh" >
        ...
        <b-row style="margin:1em;"  class="text-center" >
                    ...
                    <b-col cols="2" class="p-2 text-center"   >
                        <!-- <ol style="list-style-type: circle;float:left;" class="ml-5">
                            <li><b-badge style="font-size:0.8em;color:navy;cursor:pointer;background-color: transparent;" pill > {{socket__room_name}} </b-badge></li>
                        </ol> -->
                    </b-col>
        ...
        <div id="main-container-1" style="height:80vh;overflow:auto;border :0px dashed #5F1854;" >
            ...
            <!-- <div class="row" >
                ...
                <div style="min-height:60vh;" >
                          ...
                          <table  cellpadding=3 class="center outline striped text-dark" width=100% style="margin-top:1.5em;">
                          <thead>
                          <tr class="header font-weight-bold text-primary text-center " style="font-size:0.9em;" >
                              <th class=small ></th>
                              <th class=small >COSPAR ID</th>
                              <th class=small >NORAD ID</th>
                              <th class=small >Name</th>
                              <th class=small >Object</th>
                              <th class=small >Period <br/><span style="font-size:1em;">(hours/day)</span></th>
                              <th class=small >Position</th>
                              <th class=small >Footprint</th>
                              <!-- <th  width=50>Inclination (degrees)</th>
                              <th  width=50>Apogee Height (km)</th>
                              <th  width=50>Perigee Height (km)</th>
                              <th  width=50>Eccentricity</th>
                              <th  width=50>Latest Data</th> -->
                              <th class=small >Ex.<br/><span style="font-size:1em;">(info)</span></th>
                              <th class=small >TLE Age<br/><span style="font-size:1em;">(days)</span></th>
                              </tr>
                            </thead>
                          <tbody>

                          <tr align=center v-for="(data,index) in arr" :key="index" @click="tle_edit( data )" style="cursor:pointer;" >

                            <td class="small selection_ele-2" >
                              <i class="far fa-check-square mr-2" v-if="data.selected==true"></i>
                              <i class="far fa-square mr-2" v-else ></i>
                            </td>
                            <td class="small selection_ele-2" >{{data.sat_info.international_designator}} <i class="fal fa-search ml-1 text-primary font-weight-bold icon-find"></i></td>
                            <td class="small selection_ele-2" >{{data.sat_info.norad_catalog_number}} <i class="fal fa-search ml-1 text-primary font-weight-bold icon-find"></i></td>
                            <td class="small selection_ele-2" >{{data.sat_info.name}} <i class="fal fa-search ml-1 text-primary font-weight-bold icon-find"></i></td>
                            <td class="small selection_ele-2" >{{data.sat_info.type}}</td>
                            <td class="small selection_ele-2" >{{data.sat_info.period}}</td>
                            
                            <td class="small selection_ele-2 text-primary" v-if="data.position != 0">{{parseFloat(data.position).toFixed(1)}}°</td>
                            <td class="small selection_ele-2" v-else>-</td>

                            <td class="small selection_ele-2" >
                              <span v-if="data.orbit_footprint.length>0" class="text-success font-weight-bold">{{data.orbit_footprint.length}}</span>
                              <span v-else style="color:#999">-</span>
                            </td> 
                            <td class="small selection_ele-2" >
                              <span v-if="data.content.length>0" class="text-success font-weight-bold">info</span>
                              <span v-else style="color:#999">-</span>
                            </td> 
                            <td class="small selection_ele-2" v-if="data.sat_info.tle_age<14">{{data.sat_info.tle_age}}</td>
                            <td class="small selection_ele-2 text-danger" v-else>{{data.sat_info.tle_age}}</td>
                ...
                <ol style="list-style-type: circle;float:right;" class="ml-0">
                  <li><b-badge style="font-size:0.8em;color:navy;cursor:pointer;background-color: transparent;" pill > {{socket__room_name}} </b-badge></li>
                </ol> 
    ...
    <!-- ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- -->
        <b-modal v-model="open_upload" dialog-class="modal-xl-1" centered no-close-on-backdrop hide-footer hide-header hide-header-close  title="info"   >
        ...
        <b-modal v-model="socket_monitor_dialog" size="sm-success"  centered no-close-on-backdrop hide-header hide-header-close  title="Login"  >
          <b-row class="text-center text-success" style="margin-top:1em;">  

            <b-col>   
              <span style="font-size:0.8em;margin-top:0.5em;font-weight:bold;"> Wait until data progress finished... </span> 
            </b-col>
          ...
          <b-row class="text-center text-success" style="margin-top:0.5em;">  
            ...
            <b-col>   
              <span style="font-size:0.8em;margin-top:0.5em;"> {{socket_monitor_msg}} </span> 
            </b-col>
          ...
          <template v-slot:modal-footer="{ }"> 
              ...
              <b-row class="w-100">
                ...
                <b-col sm=8>
                  ...
                  <b-button variant="success" @click="socket_monitor_dialog = false" block class="rounded-0 "

                    :disabled="socket_monitor_dialog__exit_disable"
                    outlined
  ...
  data() {
            return {    
                  ...
                  tle_type_menu_tmp : [
                        ...
                        { selected : false , name : 'Thai Area', count :0 },
                  ...
                  socket_monitor_dialog__exit_disable : true,
                  dialog__add_disable : false,
                  socket_monitor_msg : '',
                  socket__room_name : '',
    ...
    methods: {
          ...
          // async sat_info ( item ) {
           ...
           async search_tle_byName ( name ){
             ...
             if( name.length > 0 ){
                ...
                this.socket_monitor_msg = 'Retrieve data...'
                ...
                this.socket_monitor_dialog = false
          ...
          onRowClicked : async function(item, index, event) {
           ...
           fn_get_tle_by_name  : async function ( token, recipient_publickey, name ) {
              ...
              return new Promise((resolve, reject) => { 
                      ...
                      let requestBody = {                                
                          ...
                          "socket_io_room_name" : this.socket__room_name
           ...
           async fn_connect_socket () {
              ...
              let client_name = 'client-id-'+generateChallenge()+'-space-elements'
              this.socket__room_name = 'Space-Elements--ROOM-'+generateChallenge() 
              ...
              let socket = new socket_io( '', server_socket_domain, '', client_name, '', '',
                      ...
                      async ( __room_data_ ) => {  
                          ...
                          if( __room_data_.room_name == this.socket__room_name ){
                              ...
                              // if( this.socket_monitor_dialog == false ){

                              //   this.socket_monitor_dialog = true
                              //   this.socket_monitor_dialog__exit_disable = true

                              // }

                              this.socket_monitor_msg = __room_data_.data

                              // if( __room_data_.data == 'Process-Complete' ){

                              //   this.socket_monitor_dialog__exit_disable = false
                              // }else{
